// splash.jsx — Terminal Auth opening splash
const SPLASH_LINES = [
  ...
  React.useEffect(() => {
    ...
    function step() {
      ...
      if (char <= txt.length) {
        ...
        setCurLine(line);
        char++;
        T.push(setTimeout(step, char <= txt.length ? (Math.random() * 5 + 3) : 80));
      } else {
        setVisLines(v => v + 1);
        line++; char = 0;
    ...
    <div className={'splash' + (out ? ' splash--out' : '')}>
      <div className="splash-term">
        ...
        {SPLASH_LINES.map((l, i) => {
          if (i < visLines)
            return (
              <div key={i} className={'splash-line' + (l.ok ? ' splash-line--ok' : '')}>
                {l.text}
              </div>
            );
          if (i === visLines)
            return (
              <div key={i} className={'splash-line' + (SPLASH_LINES[curLine]?.ok ? ' splash-line--ok' : '')}>
                {typed}<span className="splash-cursor" />
              </div>
            );
          return null;
        })}

        <div className={'splash-reveal' + (showLogo ? ' splash-reveal--vis' : '')}>
          <Glyph size={42} />
          <div className="splash-reveal-name">GIDEON DYNAMICS</div>
          <div className="splash-reveal-tag">PERSISTENCE IN AN UNPERSISTANT WORLD</div>
        </div>
      </div>
    </div>
  );
